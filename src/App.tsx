...
export default function App() {
  ...
  const handleLoadHistory = (historyData: TimetableResult) => {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-100 flex flex-col font-sans selection:bg-[#dcfd8b] selection:text-[#0f172a]">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 space-y-12">
        ...
        {currentTab === 'home' && (
          <HomeView
            onStartPlanning={() => setCurrentTab('features')}
            onViewCatalog={() => setShowCatalog(true)}
          />
        )}
        ...
        {currentTab === 'history' && (
          <HistoryView onLoadSchedule={handleLoadHistory} />
        )}

        {currentTab === 'share' && <ShareView result={result} />}
